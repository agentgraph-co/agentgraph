"""Trust verification guard for LangChain tools.

Provides a LangChain-compatible tool wrapper that checks AgentGraph trust
scores before allowing tool execution.  Works with any BaseTool subclass.
"""
...
class TrustGuard:
    """Pre-execution trust verification for LangChain tools.
...
def trust_gated_tool(
    ...
    """Wrap a LangChain BaseTool with a trust gate.
    ...
    Example::
        ...
        from agentgraph_bridge_langchain import trust_gated_tool
    ...
    try:
        ...
        raise ImportError(
            "langchain-core is required for trust_gated_tool. "
            "Install with: pip install agentgraph-bridge-langchain[langchain]"
        )

    if not isinstance(tool, _BaseTool):
        raise TypeError(
            f"Expected a LangChain BaseTool, got {type(tool).__name__}"
        )
    ...
    orig_args_schema = tool.args_schema

    class TrustGatedTool(BaseTool):
        """A LangChain tool wrapped with AgentGraph trust verification."""

        name: str = orig_name
        description: str = orig_description
        args_schema: type | None = orig_args_schema  # type: ignore[assignment]
        ...
        def _run(self, *args: Any, **kwargs: Any) -> str:
            ...
            try:
                ...
                if loop.is_running():
                    # Can't await in a running loop — fail safe
                    logger.warning(
                    ...
                    return (
                        ...
                        f"{orig_name}. Use async invocation (ainvoke) instead."
                    )
            except RuntimeError:
                pass
        ...
        async def _arun(self, *args: Any, **kwargs: Any) -> str:
            """Async execution with trust check."""
            result = await _guard.check(repo)
            if not result.allowed:
                msg = (
                    f"[AgentGraph] Tool '{orig_name}' blocked — "
                    f"repo '{repo}' scored {result.grade} "
...
def trust_gate_tools(
    ...
    """Wrap multiple LangChain tools with trust gates.
    ...
    Example::
        ...
        safe_tools = trust_gate_tools(tools, "langchain-ai/langchain")
    """
    _guard = guard or TrustGuard(min_tier=min_tier)
