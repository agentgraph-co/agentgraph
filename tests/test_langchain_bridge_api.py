"""Tests for the LangChain bridge API endpoints.

Tests the router endpoints (config, tools, execute) — does NOT test the
LangChain bridge module directly since LangChain may not be installed.
"""
...
# ---------------------------------------------------------------------------
# GET /bridges/langchain/config
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_langchain_config(client: AsyncClient):
    """Config endpoint returns LangChain bridge metadata."""
...
@pytest.mark.asyncio
async def test_langchain_config_tool_count_matches_tools(client: AsyncClient):
...
# ---------------------------------------------------------------------------
# GET /bridges/langchain/tools
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_langchain_tools_list(client: AsyncClient):
...
@pytest.mark.asyncio
async def test_langchain_tools_have_valid_schemas(client: AsyncClient):
...
# ---------------------------------------------------------------------------
# POST /bridges/langchain/execute — requires auth
# ---------------------------------------------------------------------------
...
async def test_execute_unknown_tool(client: AsyncClient):
    ...
    token, _ = await _setup_user(client, "lc-u1@test.com", "User1")
    resp = await client.post(
        f"{BASE}/execute",
...
async def test_execute_get_feed(client: AsyncClient):
    ...
    token, _ = await _setup_user(client, "lc-u2@test.com", "User2")
    resp = await client.post(
        f"{BASE}/execute",
...
async def test_execute_search_entities(client: AsyncClient):
    ...
    token, _ = await _setup_user(client, "lc-u3@test.com", "SearchMe")
    resp = await client.post(
        f"{BASE}/execute",
        json={
            "tool_name": "search_entities",
            "arguments": {"query": "SearchMe", "limit": 10},
        },
        headers=_auth(token),
    )
    assert resp.status_code == 200
...
async def test_execute_get_trust_score_not_found(client: AsyncClient):
    ...
    token, _ = await _setup_user(client, "lc-u4@test.com", "User4")
...
async def test_execute_get_trust_score_invalid_uuid(client: AsyncClient):
    ...
    token, _ = await _setup_user(client, "lc-u5@test.com", "User5")
    resp = await client.post(
        f"{BASE}/execute",
        json={
            "tool_name": "get_trust_score",
...
async def test_execute_create_post(client: AsyncClient):
    ...
    token, _ = await _setup_user(client, "lc-u6@test.com", "User6")
    resp = await client.post(
        f"{BASE}/execute",
        json={
            "tool_name": "create_post",
            "arguments": {"content": "Hello from LangChain bridge!"},
        },
        headers=_auth(token),
    )
    assert resp.status_code == 200
...
async def test_execute_get_entity_profile(client: AsyncClient):
    ...
    token, entity_id = await _setup_user(
        client, "lc-u8@test.com", "LangChain Test User",
    )
    resp = await client.post(
        f"{BASE}/execute",
        json={
    ...
    assert data["result"]["display_name"] == "LangChain Test User"
...
async def test_execute_get_entity_profile_not_found(client: AsyncClient):
    ...
    token, _ = await _setup_user(client, "lc-u9@test.com", "User9")
    resp = await client.post(
        f"{BASE}/execute",
        json={
            "tool_name": "get_entity_profile",
...
async def test_execute_attest_entity_self(client: AsyncClient):
    ...
    token, entity_id = await _setup_user(client, "lc-u10@test.com", "User10")
    resp = await client.post(
        f"{BASE}/execute",
        json={
...
async def test_execute_attest_entity_invalid_type(client: AsyncClient):
    ...
    token, _ = await _setup_user(client, "lc-u11@test.com", "User11")
    resp = await client.post(
        f"{BASE}/execute",
        json={
            "tool_name": "attest_entity",
...
async def test_execute_default_empty_arguments(client: AsyncClient):
    ...
    token, _ = await _setup_user(client, "lc-u12@test.com", "User12")
    resp = await client.post(
        f"{BASE}/execute",
