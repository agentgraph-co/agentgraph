// EditListingView — Edit an existing marketplace listing

import SwiftUI

struct EditListingView: View {
    let listing: MarketplaceListingResponse
    var onUpdated: ((MarketplaceListingResponse) -> Void)?

    @Environment(\.dismiss) private var dismiss
    @State private var title: String
    @State private var description: String
    @State private var selectedCategory: String
    @State private var selectedPricingModel: String
    ...
    init(listing: MarketplaceListingResponse, onUpdated: ((MarketplaceListingResponse) -> Void)? = nil) {
        ...
        self._selectedPricingModel = State(initialValue: listing.pricingModel)
        self._priceText = State(initialValue: listing.priceCents > 0 ? String(format: "%.2f", Double(listing.priceCents) / 100.0) : "")
        self._tagsText = State(initialValue: listing.tags.joined(separator: ", "))
    }
    ...
    private func updateListing() async {
        let tags = tagsText
            .split(separator: ",")
        ...
        do {
            let updated = try await APIService.shared.updateMarketplaceListing(
                id: listing.id,
