"""LangChain bridge API router.

Provides endpoints for LangChain integration configuration, tool discovery,
and proxied tool execution. These endpoints do NOT require LangChain to be
installed — they expose AgentGraph capabilities in a format that LangChain
...
from src.bridges.langchain_bridge import TOOL_DESCRIPTORS, AgentGraphToolkit
from src.database import get_db
from src.models import Entity

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/bridges/langchain", tags=["bridges"])
...
class LangChainConfigResponse(BaseModel):
    ...
    tool_count: int = 0
    description: str = (
        "AgentGraph LangChain bridge — provides tools for searching entities, "
...
class ToolSchema(BaseModel):
    """Schema for a single LangChain-compatible tool."""
...
class LangChainToolsResponse(BaseModel):
    """List of available tools for LangChain integration."""

    tools: list[ToolSchema]


class ToolExecuteRequest(BaseModel):
    """Request to execute a tool from LangChain."""
...
@router.get(
    "/config",
    response_model=LangChainConfigResponse,
    dependencies=[Depends(rate_limit_reads)],
)
async def get_langchain_config() -> LangChainConfigResponse:
    """Return LangChain integration configuration.

    Public endpoint — no authentication required.
    Provides information about the LangChain bridge including available
    auth methods, base URL, and tool count.
    """
    descriptors = AgentGraphToolkit.get_tool_descriptors()
    return LangChainConfigResponse(tool_count=len(descriptors))


@router.get(
    "/tools",
    response_model=LangChainToolsResponse,
    dependencies=[Depends(rate_limit_reads)],
)
async def list_langchain_tools() -> LangChainToolsResponse:
    """List available tools for LangChain integration.

    Public endpoint — no authentication required.
    Returns tool schemas that a LangChain agent can use to discover
    ...
    return LangChainToolsResponse(tools=tools)
...
async def _execute_attest_entity(
    ...
    # Create attestation via the formal attestations table if it exists,
    # otherwise return a lightweight confirmation.
    try:
...
async def execute_langchain_tool(
    ...
    """Execute a tool from LangChain (proxied through AgentGraph).
    ...
    try:
        ...
        await log_action(
            db,
            action="bridges.langchain.execute",
            entity_id=current_entity.id,
            resource_type="langchain_tool",
        ...
        logger.error(
            "LangChain tool execution failed: %s — %s",
            body.tool_name, exc,
            exc_info=True,
        )
        return ToolExecuteResponse(
