...
| **Envoys signature/v1 v1.5** | `envoys.me/specs/signature/v1` (commit `85c63b3`) | Wire-level HTTP signature; RFC 9421; §6 dual-shape keyid resolution (DID+JSON / Envoys-native); §4.3 OPTIONAL tag parameter (back-compat `a2a-message`); §4.2 + §5.3 SHA-512 acceptance with auto-promote at ≥4KB | §14 Vectors 1-3 byte-stable across 1.4.0→1.5.0; fixtures pinned at `aeoess/aps-conformance-suite@c16aa04` |
...
4. **Should `bilateral_receipt` carry tag value `bilateral-receipt` (per Envoys §4.3) or stay absent (defaulting to `a2a-message` per back-compat)?** This is a substrate-level convention question: does explicit tagging give gateways better byte-comparison routing, or is the default-to-`a2a-message` preferable for backward compat? Leaning toward explicit tag for v0.3.2+ implementations, default-absent for back-compat verifiers.
...
- **Draft 1:** 2026-05-13 (this commit) — posted for @eriknewton review
- **Review window:** May 13-19 (Erik's offer: "happy to review for layering precision before the May 19-22 announce")
